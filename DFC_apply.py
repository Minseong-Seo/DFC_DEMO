...
# 4. Saving
# =========================================================
def make_DFC_output_path(file_path, output_dir, SOC_standby, advance_time):
    """
    DFC 결과 CSV 경로를 생성한다.
    원본 EV6 폴더가 아니라 output_dir에 저장한다.
    """
    os.makedirs(output_dir, exist_ok=True)

    base_name = os.path.splitext(os.path.basename(file_path))[0]
    ext = os.path.splitext(file_path)[1]
    advance_label = str(advance_time).replace(" ", "")

    DFC_file_name = f"{base_name}_DFC_target_{SOC_standby}_advance_{advance_label}{ext}"
    return os.path.join(output_dir, DFC_file_name)
...
# =========================================================
def main():
    df = load_bms_csv(FILE_PATH)
    df = classify_state(df)

    df, _ = apply_delayed_full_charge(
        df,
    ...
    DFC_file_path = make_DFC_output_path(
        file_path=FILE_PATH,
        output_dir=OUTPUT_DIR,
        SOC_standby=SOC_standby,
        advance_time=T_buffer,
    )
    # DFC 적용 결과를 CSV로 저장
    df.to_csv(DFC_file_path, index=False, encoding="utf-8-sig")
    print(f"DFC CSV saved to: {DFC_file_path}")
